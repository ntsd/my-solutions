...
def df2csr(wb, df, pick_hours=None):
	...
	with timer("Adding prev click times"):
	    D= 2**26
	...
	with timer("Log-binning features"):
		for fea in ['ip_day_hour_count','ip_app_count','ip_app_os_count','ip_device_count',
				'app_channel_count','next_click','ip_channel_unique','prev_click']: 
...
#for df_c in pd.read_csv('../input/train.csv', engine='c', chunksize=batchsize, 
						skiprows= range(1,4900000), sep=",", dtype=dtypes): #range(1,9308569)
	rcount += batchsize
	if rcount== 180000000:
...
df_sub.to_csv("wordbatch_fm_ftrl_180m.csv", index=False)