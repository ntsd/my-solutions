...
class Path 
{
    ...
    public Path(int cost, Point next, List<Point> path, int score) 
    {
        this.cost = cost;
        this.next = next;
        this.path = path;
...
class Player: Point
{
    ...
    {
        return this.PathScore(graph);
    }
    
    public int PathScore(Dictionary<Point, List<Point>> graph)
    {
        Path path = this.PathScoreClosetItem(graph);
        return path.score;
    }
    
    public Path PathScoreClosetItem(Dictionary<Point, List<Point>> graph)
    {
        Queue<Path> queue = new Queue<Path>();
        queue.Enqueue(new Path(0, new Point(this.x, this.y), new List<Point>(), 0));
        ...
        while (queue.Count > 0) {
            ...
            if (!seen.Contains(path_.next)) {
                ...
                foreach (Point next in graph[path_.next]) {
                    queue.Enqueue(new Path(path_.cost+1, next, new_path, path_.score));
                }
            }
            if (queue.Count == 0) {
                path_.seen = seen;
...
class Move
{
    public List<Move> next { get; set; }
    public Move prev { get; set; }
    public int score { get; set; }
    ...
    public Move(int depth, int score, string action, int turnType) 
    {
        this.depth = depth;
        this.score = score;
        this.action = action;
...
class Simulate 
{
    ...
    public string bestAction() 
    {
        ...
        turnType = Math.abs(turnType - 1);
    }
    
    List<Move> bestPush(Move m) 
    { // 28 possible
        ...
        for (int id = 0; id < 7; id++) { //id
            ...
            map.score = players[0].score(map.graph);
            if (score > bestScore) {
                bestScore = score;
                bestActionStr = "PUSH " + id + " LEFT";
            }
            ...
            score = players[0].score(map.graph);
            // Console.Error.WriteLine(id+";right;"+score);
            if (score > bestScore) {
                bestScore = score;
                bestActionStr = "PUSH " + id + " RIGHT";
            }
            ...
            if (score > bestScore) {
                bestScore = score;
                bestActionStr = "PUSH " + id + " DOWN";
            }
        }
    }
    
    List<Move> bestMove() // 4
    {
        List<Move> moves = new List<String>();
        map.buildGraph();
...
class Globals
{
    ...
    static void Main(string[] args)
    {
        ...
        while (true) {
            ...
            Console.WriteLine(sim.bestAction());
        }
    }
}
