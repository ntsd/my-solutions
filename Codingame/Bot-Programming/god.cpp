...
void intersect(vec2 c1, float r1_turns, vec2 c2, float r2_turns, std::set<vec2>* inter){
	vec2 c2toc1=c1-c2;
	...
	bool add_objective(objective& obj){
		...
		for(auto& di:obj.candidates){
			...
			if(!contracts[di].empty()){
				...
				for(auto i=inter_others.begin();i!=inter_others.end();i++)if(i->turns2(*obj.zone)>obj.radius)inter_others.erase(i);
				if(inter_others.empty()){goto merge_failed;}//not compatible (else it is)
				{
				tmp_inter_set[di]=inter_others;
				std::set<vec2> inter_new;
				intersect(d.pos,1,obj.zone->pos,obj.radius+1,&inter_new);
				for(auto c:contracts[di])intersect(c->zone->pos,c->radius+1,obj.zone->pos,obj.radius+1,&inter_new);
				for(auto i=inter_new.begin();i!=inter_new.end();i++)if(norm(*i-d.pos)>100)inter_new.erase(i);
				for(auto c:contracts[di])for(auto i=inter_new.begin();i!=inter_new.end();i++)if(i->turns2(*c->zone)>c->radius)inter_new.erase(i);
				tmp_inter_set[di].insert(inter_new.begin(),inter_new.end());

				vec2 m={0,0}; for(auto& i:tmp_inter_set[di])m=m+i; m=m/tmp_inter_set[di].size();
				//check for rounding errors
				bool all_ok=true;
				if(d.turns2(m)>1){
					std::cerr<<"Merging contracts failed when adding ("<<obj.zone->id()<<", dist "<<obj.radius<<") (point too far)\n";
					for(auto& i:tmp_inter_set[di])std::cerr<<i<<" : "<<d.turns2(i)<<std::endl;
					all_ok=false;
				}
				for(auto c:contracts[di])if(m.turns2(*c->zone)>c->radius){
					std::cerr<<"Merging contracts failed when adding ("<<obj.zone->id()<<", dist "<<obj.radius<<") (zone "<<c->zone->id()<<", dist "<<c->radius<<")\n";
					for(auto& i:tmp_inter_set[di])std::cerr<<i<<" : "<<i.turns2(*c->zone)<<std::endl;
					all_ok=false;
				}
				if(all_ok) {tmp_tasks.push_back({di,m});continue;}
				}
				merge_failed:
				if(tmp_tasks.size()>=obj.n_needed)continue;
				bool available=true;
				for(auto c:contracts[di])if(c->n_contractors==c->n_needed){available=false;break;}
				if(!available)continue;
				for(auto c:contracts[di])c->n_contractors--;
			...
			if(contracts[di].empty()){
				...
				if(tmp_inter_set[di].empty())for(float i=0;i<6;i+=1)tmp_inter_set[di].insert(d.pos+vec2({int(100*std::cos(2*M_PI*i/6)),int(100*std::sin(2*M_PI*i/6))}));
				tmp_tasks.push_back({di,obj.zone->pos});
			}
		...
		if(tmp_tasks.size()>=obj.n_needed){
			for(auto& t:tmp_tasks){
				...
				std::cerr<<"assigned drone "<<t.drone_id<<" to ("<<obj.zone->id()<<", dist "<<obj.radius<<")"<<std::endl;
			}
			obj.n_contractors=tmp_tasks.size();
			return true;
		}
		for(int di=0;di<n_drones;di++)for(auto c:saved_contracts[di]){contracts[di].push_back(c);c->n_contractors++;}
		return false;
	}
};
...
int main(){
	...
	players.resize(n_players); for(auto& p:players) p.drones.resize(n_drones); 
	zones.resize(n_zones);   for(auto& z:zones) std::cin>>z.pos.x>>z.pos.y;

	std::vector<zone> sorted_zones=zones; std::sort(sorted_zones.begin(), sorted_zones.end(), [](zone a, zone b){return a.pos.x<b.pos.x;});
	int best=0; 
	...
	vec2 lhome=(sorted_zones[0].pos+sorted_zones[1].pos+sorted_zones[2].pos)/3;
	vec2 rhome=(sorted_zones[n_zones-1].pos+sorted_zones[n_zones-2].pos+sorted_zones[n_zones-3].pos)/3;
...
while(true){
	for(auto& p:players)p.zones.clear();
	for(auto& z:zones){std::cin>>z.owner_id; if(z.owner_id!=-1){players[z.owner_id].zones.push_back(&z); players[z.owner_id].score++;}}
	for(auto& p:players){
		int i=0; 
		for(auto& d:p.drones){
			d.id=i++;
			...
			if(p.id()==my_id)std::cerr<<"drone "<<d.id<<": "<<d.pos<<std::endl;
			if(prev_pos.x<0)continue; 
			d.speed=d.pos-prev_pos;
			d.expected_dest=nullptr;
			for(int i=0;i<zones.size();i++) if(norm(d.speed)>70 && abs(det(zones[i].pos-d.pos,d.speed/norm(d.speed)))<100 && dot(zones[i].pos-d.pos,d.speed)>0){
				int dist=d.turns2(zones[i]);
				if(d.expected_dest!=nullptr && d.turns2dest<dist) continue;
				if(d.expected_dest!=nullptr);//expected_dest changed !
				d.expected_dest=&zones[i];
				d.turns2dest=dist;
			}
		}
		if(p.zones.empty()) p.zone_center=map_size/2;
		else{
			p.zone_center={0,0};
			for(auto z:p.zones)p.zone_center=p.zone_center+z->pos;
	...
	for(auto& z:zones){
		...
		for(auto& p:players) std::sort(p.drones.begin(), p.drones.end(), [z](drone a,drone b){return a.turns2(z)<b.turns2(z);});
		std::vector<std::vector<drone> > attackers(n_drones); 
		for(int di=0;di<n_drones;di++) attackers[di].push_back(players[(my_id+1)%n_players].drones[di]);
		for(int pi=2;pi<n_players;pi++)	for(int di=0;di<n_drones;di++){
			int diff=attackers[di][0].turns2(z)-players[(my_id+pi)%n_players].drones[di].turns2(z);
			if(diff>0)  attackers[di].clear();
			if(diff>=0) attackers[di].push_back(players[(my_id+pi)%n_players].drones[di]);
		}
		std::cerr<<(is_my_zone?"+":" ")<<"Zone "<<z.id();
		std::cerr<<" allies  :"; for(auto& d:players[my_id].drones)std::cerr<<std::setw(3)<<d.turns2(z);
		std::cerr<<" enemies :"; for(auto& a:attackers)std::cerr<<std::setw(3)<<a[0].turns2(z);
		std::cerr<<" contested : ";

		std::vector<objective*> curr_depends;
		//compute objectives for zone z
		for(int di=0;di<n_drones-1;di++){
			bool contested=false; for(auto& d:attackers[di])if(d.expected_dest==&z || d.turns2(z)==0){contested=true;break;}
			std::cerr<<(contested?"x ":"o ");
			if(is_my_zone){
				if(attackers[di][0].turns2(z)==attackers[di+1][0].turns2(z))continue;
				int mydi=di; 
				int diff=attackers[di][0].turns2(z)-players[my_id].drones[mydi].turns2(z);
				objective obj;
				obj.radius=std::max(attackers[di][0].turns2(z)-1,0);
				if(diff<0){is_my_zone=false;continue;}//cannot defend
				else if(diff<=1){
					while(mydi+1<n_drones && players[my_id].drones[mydi+1].turns2(z)<=obj.radius+1)mydi++;
					for(int i=0;i<=mydi;i++)if(players[my_id].drones[i].turns2(z)>=obj.radius)obj.candidates.push_back(players[my_id].drones[i].id);
				}
				else if(diff>1)continue;//no action needed
			...
			//for(int mydi=di;mydi<n_drones;mydi++){
				while(mydi+1<n_drones && players[my_id].drones[mydi].turns2(z)==players[my_id].drones[mydi+1].turns2(z))mydi++;
				int diff=attackers[di][0].turns2(z)-players[my_id].drones[mydi].turns2(z);
				objective obj;
				...
				obj.n_needed=di+1-(mydi+1-obj.candidates.size());
				objectives.push_back(obj);
				if(is_my_zone){curr_depends.clear(); curr_depends.push_back(&objectives.back());}
			}
	...
	for(auto& p:players)std::sort(p.drones.begin(),p.drones.end(),[](drone a, drone b){return a.id<b.id;});
	...
	for(auto& obj:objectives){
		...
		for(auto dep:obj.depends)if(!tmp_context.add_objective(*dep)){dep_done=false;break;}
		if(dep_done){
			for(auto dep:obj.depends)dep->done=true;
			decisions=tmp_context;
		}else{//this is not nice but...
			for(auto& o:objectives)o.n_contractors=0;
			for(int di=0;di<n_drones;di++)for(auto c:decisions.contracts[di]){c->n_contractors++;}
		}
	}
	for(int di=0;di<n_drones;di++){
		bool available=true;
		for(auto c:decisions.contracts[di])if(c->n_contractors==c->n_needed){available=false;break;}
		if(!available)continue;
		for(auto c:decisions.contracts[di])c->n_contractors--;
		decisions.contracts[di].clear();
	}
	for(auto& obj:objectives){
		std::cerr<<(obj.done?"!":" ")<<"score: "<<obj.value<<", "<<obj.n_needed<<" drone(s) needed to ";
		if(obj.zone) std::cerr<<"zone "<<obj.zone->id(); else std::cerr<<"home?";
		std::cerr<<" at dist "<<obj.radius<<" from:";
		for(auto& di:obj.candidates){
			bool contracted=false; for(auto c:decisions.contracts[di])if(c==&obj){contracted=true;break;}
			std::cerr<<" "<<(contracted?"!":" ")<<di; 
		}std::cerr<<std::endl;
	}
	for(int i=0;i<n_drones;i++)if(decisions.contracts[i].empty()){decisions.task_per_drone[i]={i,*curr_home};}

	//output results
	for(auto& t:decisions.task_per_drone)std::cout<<t.pos<<std::endl;
}
