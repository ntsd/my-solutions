...
struct Game
{
    ...
    void updateEntity(int id, int type, int x, int y, int _item)
    {
        ...
        case 0:
        case 1:
            players.at(type).updateRobot(id, p, item, type);
            break;
        case 2:
            radars.emplace_back(id, Type::RADAR, p, item, 0);
            break;
        case 3:
            traps.emplace_back(id, Type::TRAP, p, item, 0);
            break;
        default:
            assert(false);
        }
...
struct Action
{
    ...
    ostream &dump(ostream &ioOut) const
    {
        ...
        if (type == ActionType::MOVE || type == ActionType::DIG)
        {
            ioOut << " " << dest;
        }
        if (type == ActionType::REQUEST && item == Type::RADAR)
        {
            ioOut << " RADAR";
        }
        if (type == ActionType::REQUEST && item == Type::TRAP)
        {
            ioOut << " TRAP";
        }
        if (message != "")
...
array<Action, MAX_ROBOTS> getActions(Game &game)
{
    ...
    if (robot2.distance(robot1) > 3)
    {
        actions.at(2).wait("Where is Robot1?");
    }
    else
    {
        actions.at(2).wait("I'm robot2");
    }
