...
def dist_drone_zone(dr,z):
	"""Distance d'un drone à une zone en tours"""
...
def all_dist_drone_zone(drones,zones):
	"""
	Met en 3ème ligne chaque drone le tableau de la distance
	avec chaque zone
	"""
	for p in xrange(P):
		for d in xrange(D):
...
def min_drone_to_control(zones,drones,i_zone,max_turn,max_drone_limit,ret) :
	"""
	Pour une zone, trouve les possibilités de contrôle
	au cours du temps
...
#	else: 
	...
	# passe devant les attaques à plus faible nombre
...
def set_obj_to_drones(tab_i_can, drones_player):
	"""
	Trouve des objectifs et obj secondaire à chaque drone
	...
	for o in tab_i_can:
		...
		# prend d'abord les drones déjà occupés
		...
		if len(drones_ok) >= nb_min:
			...
			for i in xrange(nb_min):
				...
				# si défense et 2eme plus loin, oublie pour le drone
...
def a_step_round_back(src,dest,step=100):
	"""
	Evite de dépasser la distance 100 même avec
...
def best_point_for_two_dir(drone,z1,z2,turn1,zones):
	"""
	Trouve le meilleur point où aller
	Pour un objectif avec un nombre de tours précis,
	en prenant en compte un 2ème objectif
	"""
...
#	print >> sys.stderr, "debug: best two dirs zones ", z1, " et ", z2, " en ", turn1, " tour for ", drone[0], "x", drone[1], " dist max:", dist_max
	...
	if drone[2][z1] > turn1+2:
		print >> sys.stderr, "warn: drone trop éloigné de " + str(z1)
		return [zone1[0], zone1[1]] 

	#compare 4 possibilités : aller vers la zone 2, le point idéal,
...
def maj_scores(scores,zones):
	"""Les 2 fonctions suivantes ne sont pas encore utilisées"""
